...
You must navigate the workspace in this exact order. Each stage reads the previous stage's output and produces a new artifact in its own output/ folder .
   ...
   01-define-outcome: Start by defining the desired outcome of the prompt.
   02-define-identity: Establish the persona and perspective.
   03-define-tasks: Define clear actions, scope, and execution details.
   04-define-context: Provide necessary background and data to eliminate guessing.
   05-define-constraints: Define the "negative space" (what the AI must NOT do).
   06-define-output-format: Define the exact shape and structure of the final answer.
   07-generate-prompt: Assemble all prior artifacts into the final high-precision prompt.
   08-validate-prompt: Perform self-evaluation and identify areas of improvement. 
